...
namespace TWEUTILS {
	...
	inline uint8_t EncodeVolt(uint16_t m) {
		return uint8_t(m < 1950 ? 0 : (m > 3650 ? 255 : (m <= 2802 ? ((m-1950+2)/5) : ((m-2800-5)/10+171)) ));
	}
	 ...
	 * @class	enum_wrapper
	 *
	 * @brief	An enum wrapper.
	 *
	 * @tparam	Enum	Type of the enum.
	 */
	template<class Enum>
	class enum_wrapper
	{
	...
	public:
		enum_wrapper() : _e() {}
		enum_wrapper(const Enum e) { _e = e; }
		
		operator Enum() const { return _e; }
		Enum to_enum() const { return _e; }
		enum_wrapper& operator =(const Enum e) { _e = e; return *this;  }
		bool operator ==(const Enum e) const { return _e == e; }
		
		enum_wrapper(const T& x) { _e = static_cast<Enum>(x); }
		//inline operator int() { return static_cast<int>(_e); }
		Enum to_value() const { return _e; }
		enum_wrapper& operator = (const T x) { _e = static_cast<Enum>(x); return *this; }
		bool operator ==(const T x) const { return _e == static_cast<Enum>(x); }
	};
	...
	/**
	 * @brief	Sizeof array (might be same with _countof)
	 *
	 * @tparam	T  	Generic type parameter.
	 * @tparam	N	Type of the siz.
	 *
	 * @returns	A size_t.
	 */
	...
	// just mimic std:move
	template <class T>
	constexpr typename std::remove_reference<T>::type&& as_moving(T&& Arg_) noexcept { // forward _Arg as movable
		return static_cast<typename std::remove_reference<T>::type&&>(Arg_);
	}
