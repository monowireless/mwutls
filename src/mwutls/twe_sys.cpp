...
#ifndef ARDUINO
uint32_t g_twesys_u32TickCount_ms_on_boot;
...
void TWESYS::SysInit() {
	...
	g_twesys_u32TickCount_ms_on_boot = 0;
	g_twesys_u32TickCount_ms_on_boot = u32GetTick_ms();
#endif
}
...
extern "C" void _putchar(char character) {   // NOLINT(clang-diagnostic-reserved-identifier)
...
uint64_t TWESYS::TweLocalTime::now() {
	...
	// convert into text
	year = static_cast<int16_t>(1900 + localTime.tm_year);
	month = static_cast<int16_t>(localTime.tm_mon + 1);
	day = static_cast<int16_t>(localTime.tm_mday);
	hour = static_cast<int16_t>(localTime.tm_hour);
	minute = static_cast<int16_t>(localTime.tm_min);
	second = static_cast<int16_t>(localTime.tm_sec);
	ms = static_cast<int16_t>(milliseconds);
...
void TWESYS::TweLocalTime::set_epoch(uint64_t ep) {
	// get YYYY MM DD hh mm ss
	tm localTime{};
	time_t t = static_cast<time_t>(ep);
