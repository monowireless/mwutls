...
namespace TWETERM {
	...
	public:
		EscSeq() : _b{0x0}, _i(0), _s(0), _s_save(0), _cmdchr(0), _vals_n(0), _vals{} { }
	...
	protected:
		...
		struct _dirtyLine {
			...
			const bitmap_type DIRTY_FULL_ = 0xFFFFFFFFFFFFFFFFull;
			const bitmap_type BIT_ = 1;
#else
			typedef uint32_t bitmap_type;
			const bitmap_type DIRTY_FULL_ = 0xFFFFFFFFul;
			const bitmap_type BIT_ = 1;
#endif

			bitmap_type _dirty;

			void set_dirty(unsigned l) { _dirty |= (BIT_ << l); }
			void set_dirty_full() { _dirty = DIRTY_FULL_; }
			void clear() { _dirty = 0;  }
			bool is_dirty(unsigned l) { return !!(_dirty & (BIT_ << l)); }
			bool is_full() { return _dirty == DIRTY_FULL_; }
	...
	public:
		ITerm(uint8_t u8c, uint8_t u8l, SimpBuf_GChar* pAryLines, GChar* pBuff) :
			wrapchar(-1),					// the end idx of the lines.
			escseq(),					// the max term size
			wrap_mode(1),					// the end iex of the columns
			cursor_l(0),					// the max term size
			cursor_c(0),			
			end_l(static_cast<int16_t>(u8l - 1)),	// ary of lines buffer.
			buf_astr_screen(nullptr), 				// the raw uint8 buffer of screen mesg
			astr_screen(pAryLines, u8l, u8l),

			_raw_buffer(pBuff),
			_b_dynamic_alloc(false), max_line(u8l - 1),

			max_col(u8c - 1), max_term_line(u8l), max_term_col(u8c), dirtyLine(),
			u8OptRefresh(0), escseq_attr(0), escseq_attr_default(0), screen_mode(0),
			cursor_mode(0), _bvisible(1), _utf8_stat(0), _utf8_result(0)
		{
			// init screen buff
			// 
		...
		ITerm(uint8_t u8c, uint8_t u8l) :
			...
			cursor_c(0),
			end_l(static_cast<int16_t>(u8l - 1)), max_line(u8l - 1),
			...
			cursor_mode(0), _bvisible(1), _utf8_stat(0), _utf8_result(0)
		{
			...
			GChar* pBuff = new GChar[static_cast<ptrdiff_t>(u8c) * u8l];
	...
	protected:
		...
		uint16_t column_idx_to_vis(int16_t idx , int16_t lin) {
			unsigned cvis = 0;
			for (unsigned i = 0; i < static_cast<unsigned>(idx) && i <= max_col; i++) {
				if (i >= astr_screen[lin].length()) // blank area
					cvis++;
				else
					cvis += (TWEUTILS::Unicode_isSingleWidth(astr_screen[lin][static_cast<int>(i)].chr()) ? 1 : 2);
			}
			return static_cast<uint16_t>(cvis);
		}
		...
		uint16_t column_vis_to_idx(int16_t c_vis, int16_t lin) {
			...
			for (vis = 0, idx = 0; idx <= max_col; idx++) {
				int cwid = 1;
				if (   idx < static_cast<int>(astr_screen[lin].length())
	...
	public:
		...
		ITerm& write(char_t c) {
			const auto d = static_cast<uint8_t>(c);
			...
			if (_utf8_stat == 0) {
				// 11bit
				if ((d & 0xE0) == 0xC0) {
					_utf8_result = ((d & 0x1F) << 6);
					_utf8_stat = 1;
				}

				// 16bit
				if ((d & 0xF0) == 0xE0)	{
					_utf8_result = ((d & 0x0F) << 12);
					_utf8_stat = 2;
				}
				...
				if (_utf8_stat == 2) {
					_utf8_result |= ((d & 0x3F) << 6);
					_utf8_stat--;
				} else {
					_utf8_result |= (d & 0x3F);
					_utf8_stat = 0;
					wc = static_cast<int32_t>(_utf8_result);
				}
			}
			
			if (wc != -1) return write(static_cast<wchar_t>(wc));
			else return *this;
		}
		...
		// set screen buffer
		void set_screen_buf(GChar* ptr, uint8_t cols, uint8_t rows);
		...
		ITerm& operator ()(int col, int row) {
			move_cursor(static_cast<uint8_t>(col) , static_cast<uint8_t>(row));
			return *this;
		}
	...
	public:
		...
		virtual ~TermAttr() = default;

		inline TWE::IStreamOut& operator ()(TWE::IStreamOut& of) override {
			// just output ESC SEQ
			of('\033');
			of('[');
			if (_attr == 0) {
