...
#endif
namespace TWEUTILS {
	using z_SimpleBuffer_size_type = uint32_t;

	/**
	 * @class	z_sub_SimpleBuffer_Dynamic
	 *
	...
	public:
		z_sub_SimpleBuffer_Dynamic() : _ptr(nullptr) {}
		z_sub_SimpleBuffer_Dynamic(z_SimpleBuffer_size_type n) : _ptr(new T[n]) {}
	...
	/**
	 * @class	z_sub_SimpleBuffer_DummyStreamOut
	 *
	 * @brief	A simple buffer dummy stream out.
	 */
	class z_sub_SimpleBuffer_DummyStreamOut {
	public:
		z_sub_SimpleBuffer_DummyStreamOut& operator ()(char_t c) {return *this;}
		z_sub_SimpleBuffer_DummyStreamOut& write_w(wchar_t c) {return *this;}
	};
	...
	/// </summary>
	template <class T, class SOUT=z_sub_SimpleBuffer_DummyStreamOut, int is_string_type=0>
	class SimpleBuffer : public SOUT
	{
	public:
		typedef SimpleBuffer self_type;
		typedef z_SimpleBuffer_size_type size_type;
		...
		public:
			...
			bool operator!=(const self_type& rhs) { return ptr_ != rhs.ptr_; }
			difference_type operator-(const self_type& rhs) { return static_cast<difference_type>(ptr_ - rhs.ptr_); }
		...
		public:
			...
			bool operator==(const self_type& rhs) { return ptr_ == rhs.ptr_; }
			bool operator!=(const self_type& rhs) { return ptr_ != rhs.ptr_; }
			difference_type operator-(const self_type& rhs) { return static_cast<difference_type>(rhs.ptr_ - ptr_); }
	...
	private:
		...
		std::unique_ptr<z_sub_SimpleBuffer_Dynamic<T>> _sp;
	...
	public:
		 ...
		 */
		SimpleBuffer(size_type u16maxlen) : _u16len(0), _u16maxlen(u16maxlen), _sp(new z_sub_SimpleBuffer_Dynamic<T>((size_type)(u16maxlen + is_string_type))) {
			_p = _sp->get_pt();
		}
		...
		SimpleBuffer& operator = (const SimpleBuffer& ref) {  // NOLINT(bugprone-unhandled-self-assignment)
		 ...
		 */
		template <signed N>
		SimpleBuffer& operator = (const T(&ref)[N]) {
		...
		template <typename T_
			...
			>::type // enable_if
		> 
		SimpleBuffer& operator = (T_ p_ref)
		{
			resize(0);
		 ...
		 * @returns	A shallow copy of this object.
		 */
		SimpleBuffer& operator = (SimpleBuffer&& ref) noexcept {
			if (ref._sp) {
				int offset = static_cast<int>(ref._p - ref._sp->get_pt());
		...
		void reserve(size_type maxlen) {
			...
			if (_p && !_sp) {
				...
				std::unique_ptr<z_sub_SimpleBuffer_Dynamic<T>> buff(new z_sub_SimpleBuffer_Dynamic<T>(static_cast<size_t>(maxlen) + is_string_type));
				_u16maxlen = maxlen;
		...
		inline const wchar_t* c_str(T_ trm=0)
		{
			if (_p) {
				_p[_u16len] = trm;
				return static_cast<const wchar_t*>(_p);
			}
			else return nullptr;
		}
	...
	inline TWE::IStreamOut& SimpleBuffer<uint8_t, TWE::IStreamOut,1>::write_w(wchar_t c) {
		// UTF-8 conversion
		if (c <= 0x7F) {
			push_back(static_cast<uint8_t>(c));
		}
		else if (c <= 0x7FF) {
			push_back(0xc0 + static_cast<uint8_t>(c >> 6));
			push_back(0x80 + static_cast<uint8_t>(c & 0x3F));
		}
		else {
			push_back(0xe0 + static_cast<uint8_t>(c >> 12));
			push_back(0x80 + static_cast<uint8_t>((c >> 6) & 0x3F));
			push_back(0x80 + static_cast<uint8_t>(c & 0x3F));
		}
		return *this;
	}
	 ...
	 * @tparam	N	Array size
	 */
	template <typename T, int N, class SOUT=z_sub_SimpleBuffer_DummyStreamOut, int STR=0>
	...
	public:
		...
		inline bool resize(size_type len) { return _b.resize(len); }
	...
	// typedefs
	typedef SimpleBuffer<uint8_t, z_sub_SimpleBuffer_DummyStreamOut, 1> SmplBuf_Byte;
	typedef SimpleBuffer<uint8_t, TWE::IStreamOut, 1> SmplBuf_ByteS;
	template <int N> using SmplBuf_ByteL = SimpleBufferL<uint8_t, N, z_sub_SimpleBuffer_DummyStreamOut, 1>;
	template <int N> using SmplBuf_ByteSL = SimpleBufferL<uint8_t, N, TWE::IStreamOut, 1>;

	typedef SimpleBuffer<wchar_t, z_sub_SimpleBuffer_DummyStreamOut, 1> SmplBuf_WChar;
	typedef SimpleBuffer<wchar_t, TWE::IStreamOut, 1> SmplBuf_WCharS;
	template <int N> using SmplBuf_WCharL = SimpleBufferL<wchar_t, N, z_sub_SimpleBuffer_DummyStreamOut, 1>;
	...
	inline TWE::IStreamOut& operator << (TWE::IStreamOut& lhs, const SmplBuf_Byte& s) {
		for (const auto x : s) { lhs.operator ()(static_cast<const char_t>(x)); }
		return lhs;
	}
	...
	inline TWE::IStreamOut& operator << (TWE::IStreamOut& lhs, const SmplBuf_ByteSL<N>& s) {
		for (const auto x : s) { lhs.operator ()(static_cast<char_t>(x)); }
		return lhs;
	}
