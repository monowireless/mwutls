...
namespace TWEUTILS {
	...
	inline int strlen_vis(const wchar_t* p) {
		int len = 0;
		while(*p != 0) {
			len += Unicode_isSingleWidth(static_cast<uint16_t>(*p)) ? 1 : 2;
			++p;
		}
	 ...
	 * @brief	get visual length of string (count wide char as two)
	 *
	 * @param	str	A wchar_t to process.
	 *
	 * @returns	An int.
	 */
	inline int strlen_vis(const TWEUTILS::SmplBuf_WChar& str) {
		...
		for (const auto x : str) {
			len += Unicode_isSingleWidth(static_cast<uint16_t>(x)) ? 1 : 2;
		}
	...
	public:
		...
		inline uint16_t operator() (const char c) {
			return operator() (static_cast<uint8_t>(c));
		}
		inline uint16_t operator() (uint8_t c) {
			...
			if (_utf8_stat == 0) {
				// 11bit
				if ((c & 0xE0) == 0xC0) {
					_utf8_result = static_cast<uint16_t>((c & 0x1F) << 6);
					_utf8_stat = 1;
				}

				// 16bit
				if ((c & 0xF0) == 0xE0) {
					_utf8_result = static_cast<uint16_t>((c & 0x0F) << 12);
					_utf8_stat = 2;
				}
				...
				if (_utf8_stat == 2) {
					_utf8_result |= static_cast<uint16_t>((c & 0x3F) << 6);
					_utf8_stat--;
				}
				else {
					_utf8_result |= static_cast<uint16_t>(c & 0x3F);
					_utf8_stat = 0;
					wc = _utf8_result;
				}
			}
	...
	static inline void toUpper(std::basic_string<char>& s) {
		for (auto p = s.begin(); p != s.end(); ++p) {
			*p = static_cast<char_t>(toupper(*p)); // toupper is for char
		}
	}
	...
	static inline void toUpper(std::basic_string<wchar_t>& s) {
		for (auto p = s.begin(); p != s.end(); ++p) {
			*p = static_cast<char_t>(towupper(*p)); // towupper is for wchar_t
		}
	}
	...
	static inline void toUpper(SmplBuf_WChar& str) {
		for (auto& x : str) {
			x = static_cast<wchar_t>(toupper(x));
		}
	}
	...
	static inline void toLower(SmplBuf_WChar& str) {
		for (auto&& x : str) {
			x = static_cast<wchar_t>(towlower(x));
		}
	}
	...
	static inline bool endsWith_NoCase(const SmplBuf_Byte& str, const char_t* suffix, unsigned suffixlen) {
		if (str.size() > suffixlen) {
			...
			for (unsigned i = str.size() - suffixlen, j = 0; j < suffixlen; i++, j++) {
				b &= (towupper(str[static_cast<int>(i)]) == towupper(suffix[j]));
			}
			return b;
		}
		else {
	...
	// fixed array version
	template <int N>
	static inline bool endsWith_NoCase(const SmplBuf_Byte& str, const wchar_t(&suffix)[N]) {
		return endsWith_NoCase(str, suffix, N - 1);
	}
	...
	// fixed array version
	template <int N>
	static inline bool endsWith_NoCase(const SmplBuf_WChar& str, const wchar_t (& suffix)[N]) {
		return endsWith_NoCase(str, suffix, N-1);
	}
	...
	static inline bool beginsWith_NoCase(const SmplBuf_Byte& str, const char_t* prefix, unsigned prefixlen) {
		if (str.size() > prefixlen) {
			...
			for (unsigned i = 0, j = 0; j < prefixlen; i++, j++) {
				b &= (towupper(str[static_cast<int>(i)]) == towupper(prefix[j]));
			}
			return b;
		}
		else {
	...
	static inline bool beginsWith_NoCase(const SmplBuf_Byte& str, const char_t(&prefix)[N]) {
		return beginsWith_NoCase(str, static_cast<const char_t*>(prefix), N - 1);
	}
	...
	static inline bool beginsWith_NoCase(const SmplBuf_WChar& str, const wchar_t(&prefix)[N]) {
		return beginsWith_NoCase(str, static_cast<const wchar_t*>(prefix), N - 1);
	}
	...
	int SmplBuf_strcmp(const T& s1, const T& s2, TF CONV) { // [](typename T::value_type &x){return x;}) {
		auto i1 = s1.cbegin();
		auto i2 = s2.cbegin();
	...
	template <typename T>
	int SmplBuf_strcmp(const T& s1, const T& s2) {
		return SmplBuf_strcmp(s1, s2, [](typename T::value_type x){return x;} );
	} 

	template <typename T>
	inline void SmplBuf_Swap(typename T::iterator a, typename T::iterator b) {
	 ...
	 * @tparam	TF COMP       (T::value_type&, T::value_type&) 	Type of the value type&amp;
	...
	bool SmplBuf_Sort2(T& a, S& b, TF COMP) {
		bool b_cond = true;
		if (a.size() < 2) return false; // not necessary
	...
	static inline bool operator <(const SmplBuf_WChar& lhs, const SmplBuf_WChar& rhs) {
		return SmplBuf_strcmp< SmplBuf_WChar >(lhs, rhs) < 0;
	}
	 ...
	 *
	 * @param	buf	The first value.
	 * @param	rhs	A value to add to it.
	 *
	 */
	template <typename T1, typename T2>
	static inline void SmplBuf_WChar_add_ops(SmplBuf_WChar& buf, T1&& lhs, T2&& rhs) {
		buf << lhs;
		buf << rhs;
	}
	...
	static inline SmplBuf_WChar operator + (SmplBuf_WChar&& lhs, SmplBuf_WChar&& rhs) {
		SmplBuf_WChar buf;
		SmplBuf_WChar_add_ops(buf, lhs, rhs);
		return std::move(buf);
	}
	static inline SmplBuf_WChar operator + (SmplBuf_WChar&& lhs, SmplBuf_WChar& rhs) {
		SmplBuf_WChar buf;
	...
	static inline void unicodeMacNormalize(SmplBuf_WChar& str) {
		...
		for (auto x : work) {
			...
			case 12441: str[-1] += 1; break; // Dakuten
			case 12442: str[-1] += 2; break; // HanDakuten
			default: str.push_back(x);
			}
		}
	}
