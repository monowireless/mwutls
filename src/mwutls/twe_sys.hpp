...
#else
extern uint32_t g_twesys_u32TickCount_ms_on_boot;
#endif
...
namespace TWESYS {
	...
	static inline uint32_t u32GetTick_ms() {
		...
		return (uint32_t)timeGetTime() - g_twesys_u32TickCount_ms_on_boot;
#elif defined(__APPLE__) || defined(__linux)
		timeval time;
		gettimeofday(&time, NULL);
		long ms = (time.tv_sec * 1000) + (time.tv_usec / 1000);
		return (uint32_t)ms - g_twesys_u32TickCount_ms_on_boot;
#else
